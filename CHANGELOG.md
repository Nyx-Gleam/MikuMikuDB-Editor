...
### Changed
- **License:** starting with 2.0, the project moves from the MIT License to GPL-3.0. Versions 1.0 and 1.1 remain MIT, as originally released.
- Limited the number of performers per song to 1 (a mistake carried over from not researching the format thoroughly enough in earlier versions).
...
- Option to choose between the two variant types that exist within the game.
...
- Standalone `.pdpack` Decryptor Tool.
- Modular codebase (`core/`, `ui/`, `localization/`) replacing the single-file structure.

### Fixed
- Rewrote the preview playback and seek bar system from the ground up.
